"""
Picks a "cheap" and a "strong" model from ALLOWED_MODELS at runtime.
...
model is available, it's used for both tiers (no routing decision left to
...
def select_tiers(models: list[str]) -> dict[str, str]:
    ...
    useful because the size-in-name heuristic can be flat-out wrong
    (e.g. gpt-oss-120b is the cheapest model on Fireworks despite having
    the biggest parameter count in its name; smaller-sounding names like
    glm-5p2 can cost far more per token). Set these once you've checked
    real pricing for whatever ALLOWED_MODELS actually contains. Overrides
    are only honored if they're in ALLOWED_MODELS -- never routes to a
    model outside the list the harness gave you.
    """
    cheap_override = os.environ.get("CHEAP_MODEL_OVERRIDE", "").strip()
    strong_override = os.environ.get("STRONG_MODEL_OVERRIDE", "").strip()

    if cheap_override and strong_override and cheap_override in models and strong_override in models:
        return {"cheap": cheap_override, "strong": strong_override}
